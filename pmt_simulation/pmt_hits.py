...
class PhotonPropagation:
    ...
    def sim_pmt_hits_with_map(self, x_0, y_0, n_photons):
        ...
        for pmt, params in pmt_pos.items():
            ...
            pmt_hits[pmt] = np.random.poisson(prob * n_photons)

        return pmt_hits

    def sim_pmt_hits_with_eq(self, pmt_position, x_0, y_0, n_photons):
        ...
        return np.random.poisson(n_photons * (r_pmt ** 2) * (z_pmt ** 2) / (4 * (R ** n)), 1)
    ...
    def pmt_hits(self, mode):
        ...
        for i, (x, y, p, t) in enumerate(zip(self.x_0, self.y_0, self.n_photons, self.arr_times)):
            ...
            if (mode==0):
                hits[voxel_name] = {key: 0 for key in pmts}
                for pmt in pmts:
                  pmt_position = self.params["pmt_positions"][pmt]
                  hits[voxel_name][pmt] = int(self.sim_pmt_hits_with_eq(pmt_position, x, y, p))
            elif (mode==1):
